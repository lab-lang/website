...
If the scientific action is also new, first [declare its package interface](/docs/contributing/scientific-vocabulary). Otherwise, start with an existing action and give the method its exact qualified identity.

This API is available in the development checkout. Install that checkout's SDK using [Development setup](/docs/development-setup#python-sdk):
