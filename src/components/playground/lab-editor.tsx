...
function renderHover(markdown: string): HTMLElement {
  ...
  container.className =
    'max-w-xs px-1 py-0.5 font-sans text-[13px] leading-[1.55] text-[#f2e8db]'

  const fenceMatch = markdown.match(/```lab\n([\s\S]*?)\n```/)
  const rest = fenceMatch
    ? markdown.slice(fenceMatch.index! + fenceMatch[0].length).trim()
    : markdown

  if (fenceMatch) {
    const pre = document.createElement('pre')
    pre.className =
      'mb-1.5 rounded-md bg-black/25 px-2 py-1.5 font-mono text-[12px] text-[#93e03f]'
  ...
  if (rest) {
    ...
    paragraph.innerHTML = rest.replace(
      /`([^`]+)`/g,
      '<code class="text-[#f2e8db]">$1</code>',
    )
...
export const LabEditor = forwardRef<LabEditorHandle, LabEditorProps>(
  function LabEditor(
    {
      path,
      value,
            ...
            linter(
              (currentView) => {
                ...
                const toCm = (diagnostic: Diagnostic): CmDiagnostic => ({
                  ...
                  to: Math.max(
                    Math.min(diagnostic.span.end, length),
                    Math.min(diagnostic.span.start + 1, length),
                  ),
                  severity:
