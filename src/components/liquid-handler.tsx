 ...
 * generated protocol adds them. The specimen batches twelve identical
 * reactions across row A: one reagent-specific tip stays mounted for the whole
 * row and revisits its source only when the P20 needs a refill.
 */
const TRANSFERS = [
...
interface Frame {
  ...
  tip: number
  ms: number
  /** Reagents already delivered across the whole row when this frame starts. */
  filled: number
  /** Destination column in row A. */
  well: number
  /** Wells that have received the current reagent when this frame starts. */
  dispensed: number
  /** Liquid currently held by the P20, in microlitres. */
  tipVolume: number
}

const LAST_STEP = TRANSFERS.length - 1
const LAST_WELL = ROW_WELLS - 1
...
/*
 * A fresh tip belongs to a reagent, not to a destination. It stays above the
 * reaction wells while distributing, so it can cover all of row A without
 * touching their contents. Volumes larger than one P20 load return to the
 * source for a refill without changing tips.
 */
TRANSFERS.forEach((transfer, index) => {
  ...
  const wellsPerLoad = Math.floor(PIPETTE_CAPACITY_UL / transfer.volume)
  for (let batchStart = 0; batchStart < ROW_WELLS;) {
    const batchSize = Math.min(wellsPerLoad, ROW_WELLS - batchStart)
    const aspiratedVolume = batchSize * transfer.volume
    ...
    for (let offset = 0; offset < batchSize; offset += 1) {
      const well = batchStart + offset
      const remainingVolume = aspiratedVolume - offset * transfer.volume
      ...
      TIMELINE.push({
        ...
        well,
        dispensed: well,
        tipVolume: remainingVolume - transfer.volume,
      })
    }
...
/** The finished row holds its fill while the block runs its thermal profile. */
TIMELINE.push({
  kind: 'cycle',
  step: LAST_STEP,
  tip: LAST_STEP,
...
/*
 * The tip rests clear of the deck; each dip reaches the tube or a safe dispense
 * height above row A. A racked tip stands with only its collar proud, so the
 * mount comes down to meet that collar rather than to the rack floor, and a
 * drop clears the bin lip.
 */
const TIP_REST_Y = RAIL_Y + 126
const TIP_PROUD = 14
const DIP_SOURCE = 245 - TIP_REST_Y
const DROP_GAP = 10
const DIP_ROW = WELL_Y - TIP_REST_Y - DROP_GAP
...
const carriageMotion = (ms: number) =>
  `transform ${ms}ms cubic-bezier(0.65, 0, 0.35, 1)`
const descendMotion = (ms: number) =>
  `transform ${ms}ms cubic-bezier(0.55, 0, 0.35, 1)`
...
function frameGeometry(frame: Frame) {
  ...
  const destinationX = WELL_X + frame.well * PITCH_X
  const liquid = frame.tipVolume / PIPETTE_CAPACITY_UL
  ...
  switch (frame.kind) {
    ...
    case 'aspirate':
      return { x: sourceX, dy: DIP_SOURCE, liquid, mounted: true }
    case 'to-row':
      return { x: destinationX, dy: 0, liquid, mounted: true }
    case 'to-well':
      return { x: destinationX, dy: DIP_ROW, liquid, mounted: true }
    case 'dispense':
      return { x: destinationX, dy: DIP_ROW, liquid, mounted: true }
    case 'to-trash':
...
function frameCaption(frame: Frame): string {
  switch (frame.kind) {
    case 'home':
      return 'p20 single channel · one tip per 12-well row'
    case 'to-tips':
    case 'pick-tip':
      return 'pick up tip · 20 µL rack'
    case 'to-trash':
    case 'drop-tip':
      return '12 wells complete · eject reagent tip'
    case 'cycle':
      return '75 cycles · 37 °C 2 min → 16 °C 5 min'
    default: {
      const transfer = TRANSFERS[frame.step]
      return `${transfer.reagent} · ${transfer.volume} µL × 12`
    }
  }
}
    ...
    <g clipPath={`url(#${LABWARE_CLIP})`}>
      <g
        style={{
          ...
          transition: carriageMotion(motionMs),
        }}
      >
        <g
          style={{
            transform: `translateY(${dy}px)`,
            transition: descendMotion(motionMs),
          }}
        >
          <path
            d={TIP_PATH}
          ...
          <path
            ...
            fill={color}
            style={{
              opacity: mounted && liquid > 0 ? 1 : 0,
              transform: `scaleY(${liquid})`,
              transformBox: 'fill-box',
              transformOrigin: 'center bottom',
              transition: `transform ${motionMs}ms ease, opacity 120ms ease`,
            }}
          />
        </g>
      </g>
...
/** A touch-free dispense: the tip stays above the row while one drop falls. */
function DispenseDrop({ frame, color }: { frame: Frame; color: string }) {
  if (frame.kind !== 'dispense') return null

  const x = WELL_X + frame.well * PITCH_X
  const startY = TIP_REST_Y + DIP_ROW + 1
  const endY = WELL_Y - 1
  const duration = `${DROP_MS}ms`

  return (
    <circle cx={x} cy={startY} fill={color} r="1.45">
      <animate
        attributeName="cy"
        dur={duration}
        fill="freeze"
  ...
  return (
    ...
    >
      <rect
        ...
        rx="2"
        style={{
          transition: `height ${motionMs}ms cubic-bezier(0.55, 0, 0.35, 1)`,
        }}
        width="10"
        x="-5"
        y={RAIL_Y + 12}
    ...
    <div className="flex h-full flex-col" ref={ref}>
      <div className="flex items-center justify-between px-5 pt-3">
        <div className="flex items-center gap-2">
          ...
          <span className="micro text-[#f6ece0]/55">
            {cycling ? 'Thermocycling' : 'Golden Gate · 12 reactions'}
          </span>
        </div>
      ...
      <div className="rail min-h-0 flex-1">
        ...
        >
          ...
          {/* Each completed drop advances the current reagent across row A. */}
          {Array.from({ length: ROW_WELLS }, (_, well) => {
            const delivered = frame.filled + (well < frame.dispensed ? 1 : 0)
            const filled = delivered > 0

            return (
              <circle
                cx={WELL_X + well * PITCH_X}
                cy={WELL_Y}
                fill={
                  filled
                    ? TRANSFERS[Math.min(delivered, TRANSFERS.length) - 1].color
                    : '#f6ece0'
                }
                fillOpacity={filled ? 1 : 0.1}
                key={well}
                r={filled ? WELL_R + 0.6 : WELL_R}
                style={{
                  transition:
                    'r 180ms ease, fill 180ms ease, fill-opacity 180ms ease',
                }}
              />
            )
          })}

          <DispenseDrop color={transfer.color} frame={frame} />
          ...
          <CarriedTip
            ...
            x={geometry.x}
          />
          <Pipette dy={geometry.dy} motionMs={frame.ms} x={geometry.x} />
        </svg>
      </div>
